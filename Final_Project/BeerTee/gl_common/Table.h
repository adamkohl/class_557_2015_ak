...
class GLTable : public GLObject
{
public:
    
    GLTable(float width, float height, float length,  int rows = 10, int cols = 10 );
    ~GLTable();
...
protected:
    ...
    */
    void make_Table(float center_x, float center_y, float center_z, float width, float height,  int rows, int cols, vector<Vertex>& vp, vector<Vertex>& nv );
