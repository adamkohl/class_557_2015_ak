...
const vec3 select_color = vec3(0.0,0.65,0.0);
...
float cosAngle = 0.0;
vec3 reflectionVector = vec3(0.0,0.0,0.0);
vec3 surfaceToCamera = vec3(0.0,0.0,0.0);
...
vec4 useLight(Light light, vec4 surfacePostion, vec4 normal_transformed, vec3 normal, Material material )
{
    ...
    cosAngle = 0.0;
    max( dot(surfaceToCamera, reflectionVector), 0.0);
...
void main(void)
{
    ...
    if(select_mode==false)
    {
        ...
        vec4 finalColor = linearColor;//pow(linearColor, gamma);
